...
def pic_calculations_avg_degree(dir_out="exp3_thesis_figs/time", dir_work="paper_exp2_time_break",
    ...
    for alg in algs:
        ...
        if 'inference' in dir_work:
            ax.set_ylabel("平均每轮推理时间 (ms)", fontsize=base_size+2)
        else:
            ax.set_ylabel("平均每轮训练时间 (ms)", fontsize=base_size+2)
        ...
        fig.savefig(dir_out + "/" + file_prefix + alg + "." + file_type, dpi=400)
        plt.close()


def pic_inference_calculations_avg_degree(dir_out="exp3_thesis_figs/time", dir_work="paper_exp5_inference_full",
...
pic_calculations_avg_degree()
pic_inference_calculations_avg_degree()
