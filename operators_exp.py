...
def get_infer_operators_time(cur, outliers):
    operators = {}
    sql = "select start, end, text from nvtx_events where text == 'eval'" 
    res_sql = cur.execute(sql).fetchall()
    for res in res_sql:
    ...
    for alg in algs:
        for data in datasets:
            for var in variables:
                ...
                if infer_flag:
                    res = get_infer_operators_time(cur, outliers)
                else:
