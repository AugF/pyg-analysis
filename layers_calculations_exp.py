...
def get_layers_calculations_time(cur, outliers, alg, layer=2, infer_flag=False):
        ...
        for i in range(50):
            ...
            if infer_flag:
                ...
                for j in range(layer):
                    layers_times[j] += get_real_time(res[layer * i + j], cur)[0]
            else:
    ...
    for alg in algs:
        for data in datasets:
            ...
            for var in variables:
                ...
                res = get_layers_calculations_time(cur, outliers, alg, layer=layer, infer_flag=infer_flag) # layer实验特有
                df[var] = res
...
def run_config_exp(params):
    ...
    for alg in algs:
        ...
        for data in datasets:
            ...
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
            outliers = np.genfromtxt(outlier_file, dtype=np.int).reshape(-1)
            res = get_layers_calculations_time(cur, outliers, alg, infer_flag=infer_flag)
            df[data] = res
        pd.DataFrame(df).to_csv(out_path)
...
if __name__ == '__main__':
    import yaml
    if len(sys.argv) < 2:
        print("python layers_calculations_exp.py [yaml_file_path, config_exp]")
        sys.exit(0)
    params = yaml.load(open(sys.argv[1]))
    if 'config_exp' in sys.argv[1]:
        run_config_exp(params)
    else:
        run_layers_calculations_exp(params) 