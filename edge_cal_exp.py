...
def get_edge_time(cur, outliers, alg='gcn', layer=2, infer_flag=False):
    """
    ...
    for label in labels:
        ...
        for i in range(50):
            ...
            if infer_flag:
                # only for eval_time
                for j in range(step):
                    cost_time += get_real_time(res[step * i + j], cur)[0]
            else:
    ...
    for alg in algs:
        for data in datasets:
            ...
            for var in variables:
                ...
                res = get_edge_time(cur, outliers, alg, layer=layer, infer_flag=infer_flag)
                df[var] = res
...
def run_config_exp(params):
    ...
    for alg in algs:
        ...
        for data in datasets:
            ...
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
            outliers = np.genfromtxt(outlier_file, dtype=np.int).reshape(-1)
            res = get_edge_time(cur, outliers, alg, 2, infer_flag=infer_flag)
            df[data] = res
        pd.DataFrame(df).to_csv(out_path)
