...
def pic_calculations_avg_degree(dir_out="paper_exp2_time_break", dir_work="paper_exp2_time_break",
    ...
    for alg in algs:
        ...
        if 'inference' in dir_work:
            ax.set_ylabel("Inference Time / Epoch (ms)", fontsize=16)
        else:
            ax.set_ylabel("Training Time / Epoch (ms)", fontsize=16)
        ...
        ax2 = ax.twinx()
        ax2.set_ylabel("Proportion of Edge Calculation(%)", fontsize=16)
        line3, = ax2.plot(xticks, edge_percent, 'rs--', label='Edge Calculation Proportion')
        plt.legend(handles=[line1, line2, line3], loc="upper left", fontsize=12)
